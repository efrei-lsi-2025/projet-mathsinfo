...
export const dijkstra = async (start: Station, end: Station) => {
  console.log(`Dijkstra: Starting...`);
  let dt = new Date().getTime();

  console.log(
    `Dijkstra: From ${start.nom} to ${end.nom}: ${JSON.stringify(start)})`
  );
  ...
  for (let i in stations) {
    if (stations[i].num == start.num) {
      PCC[stations[i].num] = { station: start, time: 0 };
    } else {
      PCC[stations[i].num] = { station: undefined, time: Infinity };
    }
  }

  let stationActuelle: Station | undefined;
  while (true) {
    stationActuelle = unvisited
      .sort((a, b) => PCC[a.num].time - PCC[b.num].time)
      .shift();

    if (!stationActuelle) break;
